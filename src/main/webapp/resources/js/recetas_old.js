...
function eliminar_ingrediente(){
	...
	var url = "/formacion/api/receta/" + $idReceta + "/ingrediente/" + ingrediente_seleccionado + "";
...
//		"processData" : false,
		...
		"success" : function(result) {
			...
			//refrescar lista
			$("#ingrediente" + ingrediente_seleccionado).remove();
...
function modificar_ingrediente(id, nombre, cantidad, gluten) {
	...
	$("#modal_modificar_ingrediente_nombre").text(nombre);
	$("#id_receta").val($idReceta);
	$("#form2_cantidad").val(cantidad);
		
	$("#modal-modificar").modal();
	
	$("#btn_modificar_ingrediente").click(function(){
		console.log('se ha pulsado boton para modificar');
		...
		$.ajax(url, {
			...
			"success" : function(result) {
				...
				if(result.mensaje == null){
					...
					//refrescar lista
					$("#ingrediente" + id + "#cantidadIng").text(result.cantidad);
