...
function refrescar_lista() {
	...
	var li = '<li>##nombre##'
			...
			+ 'onClick="show_modal(\'modal-eliminar\')">'
			+ '<span class="glyphicon glyphicon-trash" aria-hidden="true"></span>'
			+ '</button>'
			+ '<button type="button" title="Botón para editar ingrediente" '
			+ 'class="btn btn-default"'
			+ 'onClick="show_modal(\'modal-editar\')">'
	...
	$("#list_ingredientes li .btn_eliminar_ingrediente").click(function() {
		...
		$("#ingrediente_eliminar").text(ingrediente_seleccionado.nombre);
...
function eliminar_ingrediente() {
	...
	$.ajax({
		...
		success : function(data) {
			console.info("Borroooooooooo ingrediente: %o",
...
function editar_ingrediente() {
	...
	$.ajax({
		...
		"type" : "update",
		success : function(data) {
			console.info("Ingrediente editado: %o",
					ingrediente_seleccionado)
			ingredientes.splice(posicion, 1);
			refrescar_lista();
		}
